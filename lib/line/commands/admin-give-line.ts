...
/**
 * Admin command to manually give product to user by LINE User ID and deduct credit
 * Usage: /give {lineUserId} {productCode}
 * Example: /give U3123f451c952d28b86866578d91ff2a5 nf7
 */
export async function handleAdminGiveByLineId(
  replyToken: string,
  lineUserId: string,
  ...
  if (!lineUserId || !lineUserId.startsWith('U')) {
    await lineClient.replyMessage(replyToken, {
      type: "text",
      text: "❌ รูปแบบ LINE User ID ไม่ถูกต้อง\nต้องขึ้นต้นด้วย 'U'",
    });
    return;
  }
  ...
  if (!targetUser) {
    await lineClient.replyMessage(replyToken, {
      type: "text",
      text: `❌ ไม่พบผู้ใช้ LINE ID: ${lineUserId}`,
    });
    return;
  }
